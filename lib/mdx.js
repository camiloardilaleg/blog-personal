...
export const getFileBySlug = async (slug) => {
    ...
    if (fs.existsSync(path.join(root, 'data', `${slug}.mdx`))){
        var mdxSource = await fs.readFileSync(path.join(root, 'data', `${slug}.mdx`), 'utf-8');
    } else if (fs.existsSync(path.join(root, 'data', `${slug}.md`))) {
        var mdxSource = await fs.readFileSync(path.join(root, 'data', `${slug}.md`), 'utf-8');
    }
...
export const getAllFilesMetadata = () => {
    ...
    return files.reduce((allPosts, postSlug) => {
        ...
        console.log(postSlug.replace(/\.mdx|\.md/g, ''));
